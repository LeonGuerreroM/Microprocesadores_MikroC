...
void main() {
  InitMain();
  //current_duty1  = 50;                        // initial value for current_duty1,2 and 3
  current_duty2  = 50;
  current_duty3  = 50;

  //pwm_period1 = PWM_TIM2_Init(200);
  pwm_period2 = PWM_TIM2_Init(500);
  pwm_period3 = PWM_TIM3_Init(400);
  ...
  PWM_TIM3_Set_Duty(current_duty3,  _PWM_NON_INVERTED, _PWM_CHANNEL2);
  ...
  while (1) {                                // endless loop
    ...
    if (GPIOA_IDR.B3) {           // button on RA3 pressed
      ...
      PWM_TIM3_Set_Duty(current_duty3,  _PWM_NON_INVERTED, _PWM_CHANNEL2); // set newly acquired duty ratio
     }
